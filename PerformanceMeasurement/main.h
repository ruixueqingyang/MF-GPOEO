...
public:
    ...
    int SummaryOut(CONFIG& Config, bool isInit){
        ...
        if(Config.isMeasureEnergy==true){
            ...
            tmpStream.str("");
            tmpStream << std::endl;
            tmpStream << "-------- CPU Energy&Power --------" << std::endl;
            tmpStream << "Energy: " << std::setw(16) << CPUEnergy << " J" << std::endl;
            tmpStream << "minPower: " << std::setw(8) << minCPUPower << " W; avgPower: " << std::setw(8) << avgCPUPower << " W; maxPower: " << std::setw(8) << maxCPUPower << " W" << std::endl;
        ...
        if (Config.isTrace == true && isInit == true) {
            ...
            // outStream << "vecCPUPower size: " << vecCPUPower.size() << std::endl;
            // output Power Threshold

            // wfr 20220613 保存偏移, 方便后续修改
            file_pos = outStream.tellp();
            // 这里写入空格 进行占位
    ...
    int output(CONFIG& Config){
        ...
        outStream.seekp(file_pos, std::ios::beg);
        outStream << "Start Time Stamp: " << std::fixed << std::setprecision(2) << StartTimeStamp << " s";
